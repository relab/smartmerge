...
do
	echo -n "pitter$Pi "
ssh pitter"$Pi" "nohup $SM/client/client -conf $SM/scripts/newList -alg=sm -opt=$1 $3 -mode=bench -contR -nclients=1 -id='$Pi' -initsize=100 -log_events -v=6 -log_dir='/local/scratch/ljehl' > /local/scratch/ljehl/rlogpi'$Pi' 2>&1 &"

ssh pitter"$Pi" "nohup $SM/client/client -conf $SM/scripts/newList -alg=sm -opt=$1 $3 -mode=bench -contR -nclients=1 -id='1$Pi' -initsize=100 -log_events -v=6 -log_dir='/local/scratch/ljehl' > /local/scratch/ljehl/rlogpi1'$Pi' 2>&1 &"
done
...
if ! [ "$2" == "" ]; then
	...
	$SM/client/client -conf $SM/scripts/newList -alg=sm -mode=exp -rm -nclients="$2" -initsize=100 -gc-off -elog -all-cores -v=6 -log_dir='/local/scratch/ljehl' > /local/scratch/ljehl/reconflog 2>&1
else
