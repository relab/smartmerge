...
if ! [ "$*" == "" ]; then
	...
	client -conf $SM/scripts/newList -alg=sm -mode=exp -rm -nclients="$*" -initsize=100 -gc-off -elog -all-cores -v=6 -log_dir='/local/scratch/ljehl' > /local/scratch/ljehl/reconflog 2>&1
else
